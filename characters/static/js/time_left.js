function TimeLeft(char, least) {
    ...
    this.percent = 0;
    var thisObject = this;
    this.showCircle = function() { $(".radial-progress").css('display', 'block');
        this.progressIt(this.percent); };
    this.hideCircle = function() { $(".radial-progress").css('display', 'none'); };
    this.progressIt = function(progress) {
        if (this.char == charListContainer.char) $('.radial-progress').attr('data-progress', 100 - progress); };
